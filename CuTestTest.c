...
static void X_CompareAsserts(CuTest* tc, const char* file, int line,
	const char* message, const char* expected, CuString *actual) {
...
void TestCuSuiteRun(CuTest* tc) {
	...
	CuStringDelete(tc1.message);
	CuStringDelete(tc2.message);
	CuStringDelete(tc3.message);
	CuStringDelete(tc4.message);
	free(tc1.name);
	free(tc2.name);
	free(tc3.name);
	free(tc4.name);
